...
void Client::ThirdPerson() {
	...
	if (g_csgo.m_input->CAM_IsThirdPerson()) {
		float target_yaw = m_real_angle.y;
		float target_pitch = m_real_angle.x;
		float target_roll = m_real_angle.z;
		float current_yaw = *reinterpret_cast<float*>(uintptr_t(g_cl.m_local) + 0x31C4 + 0x4); // Current yaw angle
		float current_pitch = *reinterpret_cast<float*>(uintptr_t(g_cl.m_local) + 0x31C4); // Current pitch angle
		float current_roll = *reinterpret_cast<float*>(uintptr_t(g_cl.m_local) + 0x31C4 + 0x8); // Current roll angle

		// Calculate the difference between the target yaw angle and current yaw angle
		float yaw_diff = target_yaw - current_yaw;
		float roll_diff = target_roll - current_roll;
		...
		// Apply smoothing to the yaw angle
		current_roll += roll_diff * smoothing_factor;

		// Apply smoothing to the yaw angle
		current_pitch += pitch_diff * smoothing_factor;

		// Update the smoothed yaw angle
		*reinterpret_cast<float*>(uintptr_t(g_cl.m_local) + 0x31C4 + 0x4) = current_yaw;
...
void Client::StartMove(CUserCmd* cmd) {
	...
	if (g_menu.main.aimbot.enable.get())
		cmd->m_tick += game::TIME_TO_TICKS(m_lerp);
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(colors::burgundy, XOR("(ls-fiesta) "));
...
int Client::GetNextUpdate() const {
	// since we are a tick behind, lets move time forward.
...
void Client::MouseFix(CUserCmd* cmd) {
	...
	auto FixDelta = [this](float deltaValue, ConVar* sensitivity, ConVar* m_axis) -> short {
		float sensitivityValue = sensitivity->GetFloat();
		float mAxisValue = m_axis->GetFloat();

		int finalDelta = static_cast<int>((deltaValue / mAxisValue) / sensitivityValue);

		finalDelta = std::clamp(finalDelta, -32768, 32767);
		if (finalDelta == 0) {
			finalDelta = deltaValue > 0 ? 1 : -1;
		}

		return static_cast<short>(finalDelta);
		};

	if (delta.x != 0.f) {
		static ConVar* m_pitch = g_csgo.m_cvar->FindVar(HASH("m_pitch"));
		cmd->m_mousedy = FixDelta(delta.x, sensitivity, m_pitch);
	}

	if (delta.y != 0.f) {
		static ConVar* m_yaw = g_csgo.m_cvar->FindVar(HASH("m_yaw"));
		cmd->m_mousedx = FixDelta(delta.y, sensitivity, m_yaw);
	}
